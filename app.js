...
function isLocalIdColumn(header = "") {
  ...
  return (
    ...
    name.includes("invoice_no") ||
    name.includes("invoice_number")
  );
}
...
function metrics() {
  ...
  if (
    analytics?.kpis &&
    Array.isArray(analytics.kpis) &&
    analytics.kpis.length
  ) {
    const apiMetrics = analytics.kpis
      .slice(0, 4)
      .map(kpi => [
        kpi.name || "Metric",
  ...
  /* Local fallback if the Render service is unavailable. */
  let numeric = [];

  if (ds?.headers && ds?.rows) {
    ds.headers.forEach((header, index) => {
      if (
        ds.types?.[index] === "number" &&
        !isLocalIdColumn(header)
      ) {
  ...
  return [
    [
      "Total Records",
      (ds?.rows?.length || 0).toLocaleString(),
      "Dataset"
    ],
    [
...
function trendSVG() {
  ...
  if (pointsData.length < 2) {
    return `
      <div class="empty">
        Trend values are not numeric enough to chart.
      </div>
    `;
  }
  ...
  return `
    ...
    >
      <div>
        <span>${esc(firstPeriod)}</span>
        <b>${formatAnalyticsNumber(values[0], trend?.metric)}</b>
      </div>

      <div>
        <span>${esc(lastPeriod)}</span>
        <b>${formatAnalyticsNumber(values[values.length - 1], trend?.metric)}</b>
      </div>
    </div>
  `;
}
...
function categoryHTML() {
  const analytics = currentAnalytics();
  const category = analytics?.category_mix;
  ...
  const values = category.values.slice(0, 5);
  const colors = [
    "#2d7ff9",
    "#35d28d",
    "#7047eb",
  ...
  const gradientParts = values.map(
    (item, index) => {
      const pct = Math.max(
        0,
  ...
  return `
    <div
      style="width:115px;height:115px;border-radius:50%;margin:18px auto;position:relative;background:conic-gradient(${gradientParts.join(",")})"
    >
      <div
        style="position:absolute;inset:28px;border-radius:50%;background:var(--panel2)"
      ></div>
    </div>
    ...
    <div class="legend">
      ${values.map((item, index) => `
        <div>
          <span>
            <span
              style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${colors[index % colors.length]};margin-right:5px"
            ></span>
            ${esc(item.label ?? "Unknown")}
          </span>
    ...
    >
      Grouped by: ${esc(category.dimension || "Category")}
    </p>
  `;
}
...
function driversHTML() {
  const analytics = currentAnalytics();
  const drivers = analytics?.top_drivers;

  if (
    !Array.isArray(drivers?.values) ||
  ...
  return `
    <div style="overflow:auto">
      <table class="table">
        <thead>
          <tr>
            <th>${esc(drivers.dimension || "Group")}</th>
            <th>${esc(drivers.metric || "Value")}</th>
          </tr>
        </thead>
    ...
    <div class="metrics-grid">
      ${metrics().map(metric => `
        <div class="card flat metric">
          ...
          <strong>${esc(metric[1])}</strong>
          <div class="delta">${esc(metric[2])}</div>
        </div>
      `).join("")}
    </div>
        ...
        <table class="table">
          <tbody>
            ...
            <tr>
              ...
              <td>
                ${
                  state.dataset?.backendDatasetId
                    ? "✅ Connected"
                    : "Local"
                }
              </td>
            ...
            <tr>
              ...
              <td>
                ${analytics ? "✅ Live" : "Local fallback"}
              </td>
            </tr>

            <tr>
              <td>Data Quality</td>
              <td>${state.dataset?.quality ?? 0}%</td>
            </tr>

            <tr>
              <td>Domain</td>
              <td>
                ${esc(state.dataset?.domain || "Business")}
              </td>
            </tr>
          </tbody>
        </table>
...
function bindLandingActions() {
  ...
  const tryDemo = $("#tryDemo");
  const demoBuild = $("#demoBuild");

  if (landingStart && !landingStart.dataset.bound) {
    landingStart.dataset.bound = "1";
  ...
  if (heroStart && !heroStart.dataset.bound) {
    heroStart.dataset.bound = "1";
  ...
  if (tryDemo && !tryDemo.dataset.bound) {
    tryDemo.dataset.bound = "1";
    tryDemo.addEventListener(
      "click",
      loadDemo
    );
  }

  if (demoBuild && !demoBuild.dataset.bound) {
    demoBuild.dataset.bound = "1";
    demoBuild.addEventListener(
      "click",
      () => {
        loadDemo();
        createProject(
